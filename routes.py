...
def admin():
    ...
    try:
        # Get recent sessions for management
        recent_sessions = ProcessingSession.query.order_by(ProcessingSession.upload_time.desc()).limit(10).all()

        # Get system statistics
        total_sessions = ProcessingSession.query.count()
        total_records = EmailRecord.query.count()
        total_rules = Rule.query.count()
        total_whitelist_domains = WhitelistDomain.query.count()

        # Get keyword statistics
        total_keywords = AttachmentKeyword.query.count()
        active_keywords = AttachmentKeyword.query.filter_by(is_active=True).count()

        stats = {
            'total_sessions': total_sessions,
            'total_records': total_records,
            'total_rules': total_rules,
            'total_whitelist_domains': total_whitelist_domains,
...
def cases(session_id):
    ...
    # Get comprehensive statistics for Executive Summary
    # Total records in session
    total_all_records = EmailRecord.query.filterby(session_id=session_id).count()
